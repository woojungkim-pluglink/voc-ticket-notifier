> # 현재 운영 상태 (2026-09-17 15:40 기준) — 이 문서보다 우선
>
> **VOC 알림은 GitHub Actions 에서 돌고 있다.** (같은 날 오전 로컬로 임시 복구했다가 오후에 복귀 완료)
>
> | 구성요소 | 상태 |
> |---|---|
> | GitHub 워크플로우 `voc-tickets.yml` | **active** |
> | 리포 공개 범위 | **public** — Actions 무제한 무료. 결제 차단 중에도 실행됨(실측) |
> | n8n 트리거 `bgVDfgZcJvaB86GW` | active, **10분 주기**(`*/10 * * * *`) — 실제 알림 주기를 만드는 주체 |
> | 워크플로우 내장 cron | `0 * * * *` — n8n 정지 대비 **성긴 폴백**(GitHub 스케줄은 실측 하루 3~4회) |
> | 로컬 `Pluglink_VOC_Notify`/`_Summary` | **Disabled** (폴백으로 보존. 되살릴 때는 state 봉합 필수 — 아래) |
> | 과거 실행 이력·캐시 | 전량 삭제(1,548 runs / 865 caches) — public 전환 전 정리 |
> | private 자동화 2종 | `service-init-audit`·`charge-test-autofill` 은 **여전히 차단**. 10/1 한도 리셋 대기 |
>
> ## 전환 당시 검증 기록 (2026-09-17)
>
> - 시드 실행: 캐시를 비운 상태로 첫 실행 → 8명 전원 **스냅샷 초기화, 알림 0건**(의도된 동작)
> - 1분 뒤 n8n 실행: 초기화 없이 정상 감지 → 캐시 저장·복원 체인 정상
> - 전환 공백(로컬 마지막 15:25 ~ 시드 15:39)에 생성된 티켓 19건 전수 확인 → **삼켜진 알림 대상 0건**
>   (그 구간 RECEIVED 2건은 담당자 미지정이라 원래 대상 아님)
>
> ## 왜 이렇게 됐나
>
